-- Enhance public.get_session_validation to also return:
--   assessment_type (from scale_assignments or session metadata)
--   clinic_name     (Casa Branca Saúde)
-- The previous version only returned patient initials + clinician creds.
-- DROP + CREATE because the return TABLE columns are changing (CREATE OR
-- REPLACE cannot alter a TABLE return type).

DROP FUNCTION IF EXISTS public.get_session_validation(uuid);

CREATE FUNCTION public.get_session_validation(p_session_id uuid)
RETURNS TABLE(
  session_id uuid,
  status text,
