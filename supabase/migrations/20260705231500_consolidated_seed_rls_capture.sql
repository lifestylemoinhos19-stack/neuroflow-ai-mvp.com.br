-- Consolidated idempotent migration: seed user, capture_method column, and RLS policies
-- Safe to run multiple times

-- 1. Ensure seed user exists with password Skip@Pass
DO $$
DECLARE
  new_user_id uuid;
BEGIN
...
-- 2. Ensure capture_method column exists on focus_sessions
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 4. Ensure focus_sessions policies (SELECT, INSERT, UPDATE)
...
-- 5. Ensure focus_biofeedback_logs policies (SELECT, INSERT, UPDATE)
