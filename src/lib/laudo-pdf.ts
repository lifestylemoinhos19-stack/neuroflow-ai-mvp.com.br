...
export async function generateLaudoPDF(input: LaudoInput): Promise<void> {
  ...
  if (report.json.instrumentos && report.json.instrumentos.length > 0) {
    const colX = [marginX, marginX + 50, marginX + 78, marginX + 112, pageWidth - marginX]
    const tableHeaders = ['Instrumento', 'Data', 'Pontuação', 'Classificação']
    ...
    const renderTableHeader = () => {
      ...
      doc.setFontSize(8)
      doc.setTextColor(primary[0], primary[1], primary[2])
      tableHeaders.forEach((cell, idx) => {
        doc.text(cell, colX[idx] + 2, y + 4.2)
      })
      doc.setDrawColor(secondary[0], secondary[1], secondary[2])
      doc.setLineWidth(0.3)
    ...
    const drawTableRow = (cells: string[]) => {
      const wrappedCells = cells.map((cell, idx) => {
        const maxW = colX[idx + 1] - colX[idx] - 2.5
      ...
      wrappedCells.forEach((lines, idx) => {
        doc.text(lines, colX[idx] + 2, y + 4.0)
      })
      doc.setDrawColor(secondary[0], secondary[1], secondary[2])
  ...
  if (report.riscoIminente) {
    const alertText = sanitizePdfText(report.riscoIminente)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8)
