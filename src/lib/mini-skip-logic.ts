...
export function getVisibleQuestions(module: MiniModule, answers: MiniAnswers): MiniQuestion[] {
...
export function clearHiddenAnswers(module: MiniModule, answers: MiniAnswers): MiniAnswers {
  const visibleKeys = new Set(getVisibleQuestions(module, answers).map((q) => q.key))
  const cleaned = { ...answers }
  module.questions.forEach((q) => {
    if (!visibleKeys.has(q.key)) {
